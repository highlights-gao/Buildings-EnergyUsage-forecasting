...
for i in range(7*24):
    data_socket['value_socket{}'.format(i)] = data_socket['value_socket'].shift(-i-1)
data_socket.dropna(inplace=True)


#划分训练集，测试
targets = [item for item in data_socket.columns if 'value_socket' in item]
...
Did not expect the data types in the following fields: Unnamed: 0, 日期, time
'''
...
for i in range(7*24):
    data_light['value_light{}'.format(i)] = data_light['value_light_'].shift(-i-1)
...
for day in range(7): 
    day_list = []
    for i in range(dat*24,(day+1)*24):
...
data_air = data.copy()
for i in range(7*24):
    data_air['value_air{}'.format(i)] = data_air['value_air'].shift(-i-1)
